...
This repository contains a fully automated, GitOps-driven deployment for a PlayStation Network (PSN) data exporter. It runs on a lightweight Kubernetes (K3s) cluster managed by FluxCD, and pushes metrics directly to Grafana Cloud.
...
Running standard Kubernetes and GitOps controllers on 1GB of RAM usually results in Out-Of-Memory (OOM) crashes. To solve this, this repository implements three critical optimizations:
...
2. **Native Kubernetes Secrets**: Hosts the Grafana API Key. We fallback to native secrets for the secondary token to save K3s API CPU cycles.
...
graph TB
    ...
    subgraph "Oracle Cloud VM (1 OCPU, 1 GB RAM + 15GB Swap)"
        subgraph "K3s Cluster (Lightweight)"
            FLUX["FluxCD (Lightweight GitOps)"]
...
### 2. Bootstrap the VM
Once the VM is created, SSH into it and run the bootstrap script to create the Swap file and install K3s and FluxCD:
```bash
...
FluxCD will automatically detect the new image and roll it out to the K3s cluster.
